...
class TaskCollectionsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var dataService: TaskService<OnlineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([TaskServiceFmwk.TaskCollections]?, Error?) -> Void) -> Void)?
    private var entities = [TaskServiceFmwk.TaskCollections]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let taskCollections = entities[indexPath.row]

        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ID", value: "\(taskCollections.id!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! TaskCollectionsDetailViewController
...
extension TaskCollectionsMasterViewController: TaskServiceEntitySetUpdaterDelegate {
