...
class TaskPrioritiesTextsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var dataService: TaskService<OnlineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([TaskServiceFmwk.TaskPrioritiesTexts]?, Error?) -> Void) -> Void)?
    private var entities = [TaskServiceFmwk.TaskPrioritiesTexts]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let taskPrioritiesTexts = entities[indexPath.row]

        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "locale, code", value: "\(taskPrioritiesTexts.locale!), \(taskPrioritiesTexts.code!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        loadEntitiesBlock! { entities, error in
            ...
            self.entities = entities!.sorted(by: { ($0.locale!, $0.code!) < ($1.locale!, $1.code!) })
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! TaskPrioritiesTextsDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = entities[(tableView.indexPathForSelectedRow?.row)!].locale ?? ""
...
extension TaskPrioritiesTextsMasterViewController: TaskServiceEntitySetUpdaterDelegate {
