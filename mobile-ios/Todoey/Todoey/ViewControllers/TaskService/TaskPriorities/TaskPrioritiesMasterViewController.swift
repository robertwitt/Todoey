...
class TaskPrioritiesMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var dataService: TaskService<OnlineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([TaskServiceFmwk.TaskPriorities]?, Error?) -> Void) -> Void)?
    private var entities = [TaskServiceFmwk.TaskPriorities]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let taskPriorities = entities[indexPath.row]

        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "code", value: "\(taskPriorities.code!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        loadEntitiesBlock! { entities, error in
            ...
            self.entities = entities!.sorted(by: { ($0.code!) < ($1.code!) })
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! TaskPrioritiesDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(entities[(tableView.indexPathForSelectedRow?.row)!].code != nil ? "\(entities[(tableView.indexPathForSelectedRow?.row)!].code!)" : "")"
...
extension TaskPrioritiesMasterViewController: TaskServiceEntitySetUpdaterDelegate {
